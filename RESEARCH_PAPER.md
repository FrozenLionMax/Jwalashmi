...
We present **JWALASHMI** (Sanskrit: *Jwala* — flame, *Rashmi* — ray of light), a deep learning system for solar flare forecasting using multi-instrument X-ray data from ISRO's Aditya-L1 mission. By fusing soft X-ray observations from SoLEXS (1–25 keV) with hard X-ray data from HEL1OS (10–150 keV), we construct 12 physics-informed features including the hard-to-soft X-ray ratio, Neupert effect derivative, and spectral hardness index. Our system employs a two-tier architecture: a **10-model tactical ensemble** for 30-minute forecasting (77.8% balanced accuracy, 5-class) and a **5-model strategic ensemble** for 12-hour outlook (98.5% balanced accuracy). The tactical tier achieves **M-class AUC of 0.997** and **X-class AUC of 0.999**, with a three-tier operational alert system attaining **97.3% accuracy for dangerous M+X class flares** (RED alerts). To our knowledge, this is the **first machine learning model to utilize Aditya-L1 SoLEXS and HEL1OS observations** for solar flare prediction, demonstrating India's capacity for indigenous space weather forecasting. The system is deployed as a real-time mission control dashboard with live NOAA space weather integration, geomagnetic impact mapping, and 30-satellite tracking.
